...
import numpy as np
from constants import i2l_dic,el2i,B_label,S_label,len_sentence
...
def label2answer(predict_label,tokens):
    ...
    输出： python-list  3d，第三维为每个实体的起始位置列表[begin，end]（含头含尾）
          python-list  [{0:[3,4,5],1:[7]..}..],每句话为一个dic,键为实体的次序，值为该实体对应token索引的列表
    '''
    pre_entity = []#列表 存放预测出的实体位置及类别
    pre_e2t_c = []#列表 存放预测出的 化学物在句中的次序 to 对应词索引的映射
    pre_e2t_g = []#列表 存放预测出的 基因在句中的次序 to 对应词索引的映射
    ...
    for i in range(len(tokens)):
        ...
        while(j < num_token):
            ...
            if predict_label[i][j] in S_label:#如果类别为S 则直接将该token的位置加入列表
                ...
                for endj in range(j,num_token):#顺序向下遍历到第endj个token不属于实体
                ...
                elabel = tlabel.split('-')[1]#切分出实体的类别
                sen_entity.append([begin,end,elabel])
                tokenindexs = [ x for x in range(j,endj)]#存放该实体对应的token索引列表
                if elabel == 'chem':
                    sen_e2t_c[index] = tokenindexs
                    index += 1
                else:
                    sen_e2t_g[index] = tokenindexs
                    index += 1
                j = endj
            else:
                j+=1
        ...
        pre_e2t_c.append(sen_e2t_c)
        pre_e2t_g.append(sen_e2t_g)
    return pre_entity,pre_e2t_c,pre_e2t_g
