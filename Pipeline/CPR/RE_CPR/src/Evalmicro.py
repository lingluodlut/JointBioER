...
def eval_mulclass(ans_matrix, res_matrix, real=True):
    confuse_matrixs = np.zeros((ans_matrix.shape[1], 4))#五种类别分别的TP FN FP TN
    
    if real == True:#将预测出的Y转变为0/1
        res_matrix = change_real2class(res_matrix)
    
    class_indexs = np.argmax(ans_matrix, 1)#取最大的类别
    for class_index in range(confuse_matrixs.shape[0]):#对于5种类别
    ...
    TP,FN,FP,TN = .0,.0,.0,.0				
    for class_index in range(confuse_matrixs.shape[0]-1):
        for i in range(ans_matrix.shape[0]-1):
            if class_index == class_indexs[i]: #positive entry
                if res_matrix[i][class_index] == positive:
                    TP += 1 #TP
                else:
    ...
    for i in range(confuse_matrixs.shape[0]-1):
        ...
        print 'F1:    ', 2*p*r/(p+r)
        print        
    P_MACRO /= (confuse_matrixs.shape[0]-1)
    R_MACRO /= (confuse_matrixs.shape[0]-1)
    F1_MACRO = 2*P_MACRO*R_MACRO/(P_MACRO+R_MACRO)
    print 'MACRO Evaluation for all the class'
    print 'P_MACRO:    ', P_MACRO
    print 'R_MACRO:    ', R_MACRO
    print 'F1_MACRO:    ', F1_MACRO
    print
#	
#    P_MICRO, R_MICRO = .0, .0 #MICRO_F1
#    P_MICRO = TP/ (TP+FP)
#    R_MICRO = TP/ (TP+FN)
#    F1_MICRO = 2*P_MICRO*R_MICRO/(P_MICRO+R_MICRO)
#    print 'MICRO Evaluation for all the class'
