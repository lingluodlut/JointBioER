    ...
    for i in range(len(pre_elabel)):#每个句子
        ...
        for j in range(len(pre_elabel[i])):#每个实体的首token
            ...
            if len(infors)>2:#如果是包含关系的实体
                rlabel = infors[2]#当前实体关系类型
                rdir = infors[3]#当前实体的关系位置
                newlabel = []#可以与之配对的标签
                ...
                for r in matchr:
                    for d in matchd:
                        newlabel.append(r+'-'+d)
                for k in range(1,len(pre_elabel[i])):#同时向两个方向寻找匹配实体
                    if j-k >= 0 and len(pre_elabel[i][j-k].split('-')) == 4:
                        if pre_elabel[i][j-k][-4:] in newlabel:
                            if [j-k,j] not in exist_epair:
                                if rlabel == 'MU':
                                    if pre_elabel[i][j-k][-4:-2] !='MU':
                                        rlabel = pre_elabel[i][j-k][-4:-2]
                                    else:
                                        rlabel = 'C3'
                                sen_relation.append([pre_entity[i][j-k][0],pre_entity[i][j-k][1],pre_entity[i][j][0],pre_entity[i][j][1],rlabel])
                                exist_epair.append([j-k,j])
                            break
#                            else:
#                                continue
                    if j+k < len(pre_elabel[i]) and len(pre_elabel[i][j+k].split('-')) == 4:
                        if pre_elabel[i][j+k][-4:] in newlabel:
                            if [j,j+k] not in exist_epair:
                                if rlabel == 'MU':
                                    if pre_elabel[i][j+k][-4:-2] !='MU':
                                        rlabel = pre_elabel[i][j+k][-4:-2]
                                    else:
                                        rlabel = 'C3'
                                sen_relation.append([pre_entity[i][j][0],pre_entity[i][j][1],pre_entity[i][j+k][0],pre_entity[i][j+k][1],rlabel])
                                exist_epair.append([j,j+k])
                            break
#                            else:
#                                continue
...
def samer(pre,gold):#判断两个关系是否等价
    if cmp(pre,gold) == 0:
        return 1
    elif pre[4] == gold[4] and pre[0] == gold[2] and pre[1] == gold[3] and pre[2] == gold[0] and pre[3] == gold[1]:
        return 1
    else:
        return 0
            ...
            for pre in pre_relation[i]:
                for gold in gold_relation[i]:
                     ...
                     if samer(pre,gold)== 1:   
                        truenum +=1
                        break
    print 'goldr, prer, truer: %d %d %d'%(goldnum,prenum,truenum)
    try:
        precise = float(truenum) / float(prenum)
        recall = float(truenum) / float(goldnum)
        f = float(2 * precise * recall /( precise + recall)) 
    ...
    for i in range(len(gold_relation)):
        ...
        if len(gold_relation[i]) == 0 or len(pre_relation[i]) == 0:        
            ...
            for j in range(len(gold_relation[i])):
                ...
                for pre in pre_relation[i]:
                    #if pre[0] == gold[0] and pre[1] == gold[1] and pre[2] == gold[2] and pre[3] == gold[3] and pre[4] == gold[4]:
                    if samer(pre,gold) and Relationlabel[i][j]==1:   
                        truenum1 += 1
                        break
                    elif samer(pre,gold) and Relationlabel[i][j]==0:   
                        truenum2 += 1
                        break
#    print truenum1,truenum2
#    print goldnum1,goldnum2
    try:
        r1 = float(truenum1)/float(goldnum1)#重叠关系的召回率
        r2 = float(truenum2)/float(goldnum2)#非重叠关系的召回率
    except:
        r1 = r2 = 0.0
    return r1,r2
